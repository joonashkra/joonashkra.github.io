...
function Projects({ content, setContent }: ProjectsProps) {
  ...
  return (
    <div
      className={`flex flex-col justify-start md:justify-center md:w-1/2 h-full gap-4 ${content === "skills" ? "md:flex hidden" : ""}`}
    >
      <div className="flex justify-between items-center">
        <h1
          ref={ref}
          className={`text-2xl lg:text-3xl relative w-[max-content] before:absolute before:inset-0 before:bg-offwhite dark:before:bg-dark ${isVisible ? "before:animate-eightStepWriter" : ""}`}
        >
          Projects
        </h1>
        <button
